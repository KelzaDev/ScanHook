--[[  SCAN & HOOK TOOL  v3.6
  Fix: cache-only approach, không dùng :IsA() trong __namecall
  Hook Tab: SimpleSpy-style UI
]]
...
local WIN=Instance.new("Frame",SG); WIN.Size=UDim2.new(0,880,0,580); WIN.Position=UDim2.new(0.5,-440,0.5,-290); WIN.BackgroundColor3=BG; WIN.BorderSizePixel=0; WIN.Active=true; WIN.Draggable=true
...
local titleL=Instance.new("TextLabel",TBar); titleL.Text="SCAN & HOOK  v3.6"; titleL.Size=UDim2.new(0,230,1,0); titleL.Position=UDim2.new(0,12,0,0); titleL.BackgroundTransparency=1; titleL.TextColor3=TXT; titleL.Font=Enum.Font.GothamBold; titleL.TextSize=13; titleL.TextXAlignment=Enum.TextXAlignment.Left; titleL.RichText=false
...
-- Search bar
local sSearch=mkTB(simBar,"search in log...",UDim2.new(1,-400,0,20),UDim2.new(0,295,0,2))
local sSrchBtn=mkB(simBar,"Find",Color3.fromRGB(22,50,90),UDim2.new(0,38,0,20),UDim2.new(1,-78,0,2))
local sSrchClr=mkB(simBar,"x",Color3.fromRGB(50,20,20),UDim2.new(0,26,0,20),UDim2.new(1,-38,0,2))
local sLog=mkLog(sP,UDim2.new(1,-8,1,-66),UDim2.new(0,4,0,62))

local function doSearch(kw)
    if kw=="" then return end; kw=kw:lower()
    local res={}; for _,line in ipairs(sLogBuf) do if line:lower():find(kw,1,true) then res[#res+1]=line end end
    sLog.clear()
...
sSrchClr.MouseButton1Click:Connect(function()
    pcall(function() sSearch.Text="" end); sLog.clear()
    for _,l in ipairs(sLogBuf) do sLog.add(l) end; sLog.flush(); setSt("Cleared search")
end)
...
local function runScan(obj,depth,label,remOnly)
    ...
    task.spawn(function()
        ...
        while i<=#buf do
            ...
            sLog.flush(); i=i+200; if i<=#buf then task.wait() end
        end
        sLog.flush(); setSt(label..": "..#buf.." lines")
    end)
end

bRS.MouseButton1Click:Connect(function() runScan(RS,6,"RS",false) end)
bWS.MouseButton1Click:Connect(function() runScan(WS,4,"WS",false) end)
bRem.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        local seen,deduped={},{}
        for _,l in ipairs(buf) do local n=l:match("%] (.+)$"); if n and not seen[n] then seen[n]=true; deduped[#deduped+1]=l end end
        table.sort(deduped)
...
bUID.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        local function s(obj,path,d)
            ...
            pcall(function() for _,c in ipairs(obj:GetChildren()) do local cn=""; pcall(function() cn=c.Name end); s(c,path.."/"..cn,d+1) end end)
        end
...
bDiff.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        local function diff(a,b,label) for k,v in pairs(b) do if not a[k] then sLog.add("[+] "..label.."/"..k.." ("..v..")"); n=n+1 end end; for k,v in pairs(a) do if not b[k] then sLog.add("[-] "..label.."/"..k.." ("..v..")"); n=n+1 end end end
        diff(diffSnap.ws,nowWS,"WS"); diff(diffSnap.rs,nowRS,"RS")
        if n==0 then sLog.add("(no changes)") end; sLog.flush(); setSt("Diff: "..n.." changes")
    end)
end)
bJSON.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        local function j(o,depth) if depth>4 then return end; pcall(function() local cn=""; pcall(function() cn=o.ClassName end); local nm=""; pcall(function() nm=o.Name end); addKV(nm,cn); pcall(function() for _,c in ipairs(o:GetChildren()) do j(c,depth+1) end end) end) end
        pcall(function() j(RS,0) end)
        local out="{"..table.concat(parts,",").."}"; pcall(setclipboard,out); sLog.add("JSON copied ("..#out.." chars)"); sLog.flush(); setSt("JSON copied")
    end)
end)
...
simBtn.MouseButton1Click:Connect(function()
    simOn=not simOn
    if simOn then
        pcall(function() simBtn.Text="Watch Sim: ON"; simBtn.BackgroundColor3=Color3.fromRGB(12,68,36) end); simCnt2=0
        task.spawn(function()
            local watched=nil
            while simOn do
                task.wait(2)
                        ...
                        if tostring(p:GetAttribute("USERID"))==tostring(LP.UserId) then
                            ...
                            simConn=sim.ChildAdded:Connect(function(child)
                                if #child.Name==36 and child.Name:match("^%x+%-%x+%-%x+%-%x+%-%x+$") then
                                    simCnt2=simCnt2+1; pcall(function() simLbl.Text="UUIDs: "..simCnt2 end); sLog.add("[NEW UUID] "..child.Name); sLog.flush()
                                end
                            end)
...
local typeBadge=mkL(rightHdr,"",UDim2.new(0,80,0,16),UDim2.new(1,-85,0,5),AMBER,Enum.Font.GothamBold,9)
typeBadge.TextXAlignment=Enum.TextXAlignment.Center; Instance.new("UICorner",typeBadge).CornerRadius=UDim.new(0,4)
typeBadge.BackgroundColor3=BG3; typeBadge.BackgroundTransparency=0
local codeSF=mkSF(rightP,UDim2.new(1,-4,1,-92),UDim2.new(0,2,0,28))
...
codeTxt.TextWrapped=true; codeTxt.RichText=false; codeTxt.Text="-- Select a call from the left"

-- Action buttons row 1
local btnRow1=mkF(rightP,BG2,UDim2.new(1,-4,0,28),UDim2.new(0,2,1,-60),5)
local bLL1=Instance.new("UIListLayout",btnRow1); bLL1.FillDirection=Enum.FillDirection.Horizontal; bLL1.Padding=UDim.new(0,4); bLL1.VerticalAlignment=Enum.VerticalAlignment.Center
local bPad1=Instance.new("UIPadding",btnRow1); bPad1.PaddingLeft=UDim.new(0,6); bPad1.PaddingTop=UDim.new(0,4)
local function rb1(t,bg) local b=mkB(btnRow1,t,bg,UDim2.new(0,0,0,22)); b.AutomaticSize=Enum.AutomaticSize.X; local p=Instance.new("UIPadding",b); p.PaddingLeft=UDim.new(0,8); p.PaddingRight=UDim.new(0,8); return b end
local bCopyCode=rb1("Copy Code",Color3.fromRGB(12,55,55))
local bCopyRemote=rb1("Copy Remote",Color3.fromRGB(12,44,72))
local bCopyArgs=rb1("Copy Args",Color3.fromRGB(14,46,38))
local bBlock=rb1("Block: OFF",Color3.fromRGB(52,16,16))
local bRunCode=rb1("Run Code",Color3.fromRGB(20,68,30))

-- Action buttons row 2
local btnRow2=mkF(rightP,BG2,UDim2.new(1,-4,0,26),UDim2.new(0,2,1,-30),5)
local bLL2=Instance.new("UIListLayout",btnRow2); bLL2.FillDirection=Enum.FillDirection.Horizontal; bLL2.Padding=UDim.new(0,4); bLL2.VerticalAlignment=Enum.VerticalAlignment.Center
local bPad2=Instance.new("UIPadding",btnRow2); bPad2.PaddingLeft=UDim.new(0,6); bPad2.PaddingTop=UDim.new(0,3)
local function rb2(t,bg) local b=mkB(btnRow2,t,bg,UDim2.new(0,0,0,20)); b.AutomaticSize=Enum.AutomaticSize.X; local p=Instance.new("UIPadding",b); p.PaddingLeft=UDim.new(0,8); p.PaddingRight=UDim.new(0,8); return b end
local bExclude=rb2("Exclude",Color3.fromRGB(52,42,12))
local bClrMod=rb2("Clr Mod",Color3.fromRGB(52,22,12))
local bClrCalls=rb2("Clr Calls",Color3.fromRGB(50,10,10))
local bTestHook=rb2("Test Hook",Color3.fromRGB(18,58,38))

-- Generate SimpleSpy-style code
local function genCode(entry)
    if not entry then return "-- no call selected" end
    local lines={"-- Script generated by SCAN & HOOK v3.6","","local args = {"}
    for i,v in ipairs(entry.a) do
        local vs; if type(v)=="string" then vs='"'..v..'"' elseif type(v)=="number" or type(v)=="boolean" then vs=tostring(v) else vs='--[['..type(v)..']] '..tostring(v) end
        lines[#lines+1]="    ["..i.."] = "..vs..","
    end
...
local function showCall(entry)
    ...
    local code=genCode(entry); local n=0; for _ in code:gmatch("\n") do n=n+1 end; n=n+1
    local h=n*13+8
    pcall(function()
...
local function addCallItem(entry)
    ...
    table.insert(callLog,1,entry); if #callLog>MAX_CALLS then table.remove(callLog) end
    ...
    it.Size=UDim2.new(1,0,0,28); it.BackgroundColor3=BG3; it.BorderSizePixel=0; it.RichText=false; it.Text=""; it.LayoutOrder=0
    ...
    cntL.TextXAlignment=Enum.TextXAlignment.Center; Instance.new("UICorner",cntL).CornerRadius=UDim.new(0,3)
    cntL.BackgroundColor3=BG2; cntL.BackgroundTransparency=0
    for _,c in ipairs(callSF:GetChildren()) do if c:IsA("TextButton") then pcall(function() c.LayoutOrder=c.LayoutOrder+1 end) end end
    it.MouseButton1Click:Connect(function()
        for _,c in ipairs(callSF:GetChildren()) do if c:IsA("TextButton") then pcall(function() c.BackgroundColor3=BG3 end) end end
        it.BackgroundColor3=Color3.fromRGB(30,30,65); showCall(entry)
    end)
    local cnt=0; for _,c in ipairs(callSF:GetChildren()) do if c:IsA("TextButton") then cnt=cnt+1 end end
    pcall(function() callSF.CanvasSize=UDim2.new(0,0,0,cnt*29+4); callSF.CanvasPosition=Vector2.new(0,0); callCountL.Text=tostring(cnt) end)
end
    ...
    while WIN.Parent do
        ...
        if #hookQueue>0 then
            ...
            for _,e in ipairs(batch) do
                pcall(function()
                    if e[1]=="F" or e[1]=="I" or e[1]=="C" then addCallItem({n=e.n,t=e[1],a=e.a or {},ts=os.clock()}) end
                end)
            end
        end
    end
...
-- Button logic
bCopyCode.MouseButton1Click:Connect(function() if not selCall then setSt("Select a call first"); return end; pcall(setclipboard,genCode(selCall)); setSt("Code copied!") end)
bCopyRemote.MouseButton1Click:Connect(function() if not selCall then setSt("Select a call first"); return end; pcall(setclipboard,'game:GetService("ReplicatedStorage").events.'..selCall.n); setSt("Remote copied!") end)
bCopyArgs.MouseButton1Click:Connect(function() if not selCall then setSt("Select a call first"); return end; pcall(setclipboard,"{"..a2s(selCall.a).."}"); setSt("Args copied!") end)
...
bRunCode.MouseButton1Click:Connect(function()
    if not selCall then setSt("Select a call first"); return end
    local code=genCode(selCall); local ok,fn=pcall(loadstring,code)
    if ok and fn then local ok2,err=pcall(fn); setSt(ok2 and "Ran!" or "Error: "..tostring(err))
...
bClrMod.MouseButton1Click:Connect(function() if not selCall then return end; cfg(selCall.n).mod=nil; setSt("Mod cleared") end)

local function doClearCalls()
    callLog={}; selCall=nil
    for _,c in ipairs(callSF:GetChildren()) do if c:IsA("TextButton") then c:Destroy() end end
    pcall(function() callSF.CanvasSize=UDim2.new(0,0,0,0); callCountL.Text="0"; codeTitle.Text="<- click a call"; typeBadge.Text=""; codeTxt.Text="-- cleared" end)
end
...
local function doTestHook()
    ...
    else lines[#lines+1]="No RemoteEvent found" end
    local h=#lines*13+8
    pcall(function() codeTxt.Size=UDim2.new(1,-6,0,h); codeTxt.Text=table.concat(lines,"\n"); codeSF.CanvasSize=UDim2.new(0,0,0,h+6); codeSF.CanvasPosition=Vector2.new(0,0); codeTitle.Text="Diagnostics"; typeBadge.Text="" end)
end
bTestHook.MouseButton1Click:Connect(doTestHook)
hTest.MouseButton1Click:Connect(doTestHook)

-- ================================================================
-- NAMECALL HOOK
-- ================================================================
local function installNC()
    if namecallInstalled then return true end
    local ok,mt=pcall(getrawmetatable,game); if not ok or not mt then setSt("[ERR] getrawmetatable failed"); return false end
    if not pcall(setreadonly,mt,false) then setSt("[ERR] setreadonly failed"); return false end
    oldNamecall=rawget(mt,"__namecall"); if not oldNamecall then pcall(setreadonly,mt,true); setSt("[ERR] no __namecall"); return false end
    buildRemoteCache()
    local cc=0; for _ in pairs(remoteCache) do cc=cc+1 end; setSt("hook ok, "..cc.." cached")
    rawset(mt,"__namecall",function(self,...)
        local method=""
        if getnamecallmethod then pcall(function() local m=getnamecallmethod(); if type(m)=="string" then method=m end end) end
        ...
        if cached then
            ...
            if capF or capI then
                local args=table.pack(...); if method~="" and args[args.n]==method then args.n=args.n-1 end
                local clean={}; for i=1,args.n do clean[i]=args[i] end
                bump(cached.nm); ensureR(cached.nm); remLastArgs[cached.nm]=clean
                local c2=cfg(cached.nm); local pass=true
                if c2.filt and c2.filt~="" then pass=false; for _,v in ipairs(clean) do if tostring(v):lower():find(c2.filt:lower(),1,true) then pass=true; break end end end
            ...
            elseif hookDebug and method~="" then hookQueue[#hookQueue+1]={[1]="D",m=method,n=cached.nm} end
...
local function removeNC()
    ...
    pcall(function() hFire.Text="Hook Fire"; hFire.BackgroundColor3=Color3.fromRGB(50,22,105); hInv.Text="Hook Invoke"; hInv.BackgroundColor3=Color3.fromRGB(22,52,105); hDbg.Text="Debug: OFF"; hDbg.BackgroundColor3=Color3.fromRGB(38,38,18) end)
    setSt("Hook removed")
end
...
hCli.MouseButton1Click:Connect(function()
    if #clientConns>0 then
        ...
        pcall(function() hCli.Text="Watch Client"; hCli.BackgroundColor3=Color3.fromRGB(90,30,30) end); setSt("Watch stopped"); return
    end
    ...
    for _,r in ipairs(evs:GetChildren()) do if r:IsA("RemoteEvent") then
        ...
        local conn=r.OnClientEvent:Connect(function(...) bump(rn); local now=tick(); if now-(clientLastLog[rn] or 0)<0.3 then return end; clientLastLog[rn]=now; local a={...}; hookQueue[#hookQueue+1]={[1]="C",n=rn,a=a} end)
...
local dHdr=mkF(dLeft,BG3,UDim2.new(1,0,0,28),UDim2.new(0,0,0,0),7); local dHF=mkF(dHdr,BG3,UDim2.new(1,0,0.5,0),UDim2.new(0,0,0.5,0)); dHF.BorderSizePixel=0
...
local dRH=mkF(dRight,BG2,UDim2.new(1,0,0,28),UDim2.new(0,0,0,0),7); local dRF=mkF(dRH,BG2,UDim2.new(1,0,0.5,0),UDim2.new(0,0,0.5,0)); dRF.BorderSizePixel=0
...
local function decompObj(obj) pcall(function() dName.Text=obj.Name; dTxt.Text="Decompiling..." end)
    task.spawn(function() local ok,r=pcall(decompile,obj)
        if ok and type(r)=="string" and #r>10 then showSrc(r); setSt("OK: "..obj.Name)
        else local ok2,r2=pcall(getscriptbytecode,obj)
            if ok2 and type(r2)=="string" and #r2>0 then showSrc("-- bytecode\n"..r2); setSt("Bytecode")
            else pcall(function() dTxt.Text="Failed: "..tostring(r) end); setSt("Failed") end end end)
end
local function buildList(flt) for _,c in ipairs(dList:GetChildren()) do if c:IsA("TextButton") then c:Destroy() end end; local n=0
    for _,s in ipairs(scriptCache) do if flt=="" or s.nm:lower():find(flt:lower(),1,true) then
        local it=Instance.new("TextButton",dList); it.Size=UDim2.new(1,-4,0,20); it.Text=s.nm; it.BackgroundColor3=BG3; it.BorderSizePixel=0; it.RichText=false
        it.TextColor3=s.ref:IsA("LocalScript") and CYAN or (s.ref:IsA("ModuleScript") and PURP or TXT1); it.Font=Enum.Font.Gotham; it.TextSize=10; it.TextXAlignment=Enum.TextXAlignment.Left
        Instance.new("UICorner",it).CornerRadius=UDim.new(0,4); local pd=Instance.new("UIPadding",it); pd.PaddingLeft=UDim.new(0,5)
        local ref=s.ref; it.MouseButton1Click:Connect(function()
    ...
    end end; pcall(function() dList.CanvasSize=UDim2.new(0,0,0,n*22+4) end)
end
dScanB.MouseButton1Click:Connect(function()
    scriptCache={}; local function f(o) for _,c in ipairs(o:GetChildren()) do if c:IsA("LocalScript") or c:IsA("ModuleScript") or c:IsA("Script") then scriptCache[#scriptCache+1]={nm=c.Name,ref=c} end; pcall(function() f(c) end) end end
...
dSrch.FocusLost:Connect(function(enter)
    ...
    for line in (decompiledSource.."\n"):gmatch("([^\n]*)\n") do ln=ln+1; if line:lower():find(kw:lower(),1,true) then out[#out+1]=string.format("%4d | %s",ln,line); n=n+1 end end
    local h=math.max(n,1)*13+8; pcall(function() dTxt.Size=UDim2.new(1,-4,0,h); dTxt.Text=n>0 and table.concat(out,"\n") or "No: "..kw; dSF.CanvasSize=UDim2.new(0,0,0,h+4) end); if n>0 then setSt(n.." matches") end
end)
dRems.MouseButton1Click:Connect(function()
    ...
    for line in (decompiledSource.."\n"):gmatch("([^\n]*)\n") do ln=ln+1; if line:find("FireServer") or line:find("InvokeServer") or line:find("FireClient") then out[#out+1]=string.format("%4d | %s",ln,line:match("^%s*(.-)%s*$")); n=n+1 end end
    local h=math.max(n,1)*13+8; pcall(function() dTxt.Size=UDim2.new(1,-4,0,h); dTxt.Text=n>0 and table.concat(out,"\n") or "No remote calls"; dSF.CanvasSize=UDim2.new(0,0,0,h+4) end); setSt("Remote calls: "..n)
end)
...
local APIS={{"getrawmetatable",function() local ok,r=pcall(function() return type(getrawmetatable(game))=="table" end); return ok and r end},{"setreadonly",function() local ok=pcall(function() local m=getrawmetatable(game); setreadonly(m,false); setreadonly(m,true) end); return ok end},{"getnamecallmethod",function() return type(getnamecallmethod)=="function" end},{"newcclosure",function() return type(newcclosure)=="function" end},{"setclipboard",function() local ok=pcall(function() setclipboard("") end); return ok end},{"decompile",function() return type(decompile)=="function" end},{"getscriptbytecode",function() return type(getscriptbytecode)=="function" end},{"gethui",function() local ok,r=pcall(gethui); return ok and r~=nil end},{"hookfunction",function() return type(hookfunction)=="function" end},{"loadstring",function() local ok,f=pcall(loadstring,"return 1"); return ok and type(f)=="function" end},{"Drawing",function() return type(Drawing)=="table" end},{"firesignal",function() return type(firesignal)=="function" end}}
exChk.MouseButton1Click:Connect(function()
    exLog.clear(); exLog.header("Executor Check"); local pass=0
    for _,ck in ipairs(APIS) do local ok,r=pcall(ck[2]); if ok and r==true then exLog.add("[OK] "..ck[1]); pass=pass+1 else exLog.add("[--] "..ck[1]) end end
